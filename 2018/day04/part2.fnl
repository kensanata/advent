...
;; Next, parse all the data. We just need to count the minutes they
;; preferred to sleep through.
(var count {})
(var id nil)
(var sleep false)
(var from nil)
      ...
      (if n
          ...
          (do
            ...
            (if sleep
                (set from minute)
                (do
                  ;; count the minutes the guard slept through
                  (let [t (or (. count id) {})]
                    (for [i from (- minute 1)]
                      (tset t i (+ 1 (or (. t i) 0))))
                    (tset count id t)))))))))
